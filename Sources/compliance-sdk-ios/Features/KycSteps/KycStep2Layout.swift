...
class KycStep2Layout: UIView {
    ...
    let infoLabel: CSLabel = {
        let v = CSLabel(
            text: "Check your IDs for the following:",
    ...
    let info2: CSLabel = {
        let v = CSLabel(
            text: "2. The details are still readable",
            style: CSTextStyles.bodyTwoRegular
        )
        v.numberOfLines = 0
        v.textColor = ComplianceSDK.shared.configuration.theme.color.sendPrimary
    ...
    let logo: UIImageView = {
        let v = UIImageView(image: UIImage(name: "kyc_logo22"))
    ...
    override init(frame: CGRect) {
        ...
        info1.setContentHuggingPriority(.defaultLow, for: .horizontal)
